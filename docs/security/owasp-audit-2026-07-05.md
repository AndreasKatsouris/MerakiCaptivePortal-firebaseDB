...
2. Re-ran `npm audit` (functions workspace) and confirmed `@grpc/grpc-js` **no longer appears** in the vulnerability list; `package-lock.json` pins it at `1.12.6`. Combined with the 2026-07-02 finding, **HIGH-08 is now formally closed** (see status change below) — two independent scans confirm the resolution and the current advisory list has no grpc-js entry at all (only the pre-existing `uuid`-chain moderate/high findings via `@google-cloud/firestore`/`gaxios`/`google-gax`/`teeny-request`/`retry-request`, unchanged, still blocked on the `firebase-admin@13` major upgrade).
...
- **HIGH-08 (`@grpc/grpc-js` DoS CVEs) → CLOSED.** Two independent scans (2026-07-02, 2026-07-05) confirm `1.12.6` is installed and `npm audit` no longer flags it. No code change needed — this was transitive lockfile drift, not an intentional fix, so no PR closes it; the backlog row is marked resolved.
