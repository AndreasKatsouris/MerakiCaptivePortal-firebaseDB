...
| P6 | **Every** tenant-CSV string reaching output (`itemCode`, `description`, `supplierName`, `criticalityReason`) truncated to 120 chars, control characters stripped | Security F4: tenant-CSV strings flow into the model conversation (and into super-admin sessions — priv-esc surface); `supplierName` is a NEW sink vs D1. `itemCode` + `criticalityReason` added during build (T4 spec review) — the named-two-fields draft under-enumerated the class |
