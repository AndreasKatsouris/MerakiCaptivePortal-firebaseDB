...
## Overview
Build a shared, provider-agnostic prepaid ZAR credit ledger: `functions/billing/ledger.js`
...
- **Seed rates are NOT in the spec text** — Anthropic USD-per-Mtok rates + `usdToZar` FX must
  be operator-provided. Parameterize; never guess. (See "Operator input required" below.)
...
4. **Cloud Functions** (`cloud-functions.js`): `billingGrantCredit` (superAdmin, validate positive int),
   `billingGetBalance` (userOrAdmin, **uid-scoped**, `fxStaleWarning` when FX > 7 days),
...
6. **Seed** (`seed-price-table.js`): standalone admin-SDK script (pattern: `scripts/grant-super-admin.js`),
   writes `billing/priceTable` = `{ fx:{usdToZar,updatedAt}, markup:1.30, models:{...} }`. Rates as
...
- CF-level: auth gates, uid-scoping (read can't be redirected via body uid), `fxStaleWarning`, validation.
...
- **ZAR-cents rounding drift** → single final `Math.round`; x.5 golden vectors; independent integer
  rounding of `wholesaleUsdCents` and `costZarCents`.
...
## Operator input required before the seed runs (not before coding)
...
- **`usdToZar` FX rate** — operator-specific; needed for the seed (the only value I genuinely can't
  source). Markup locked at **1.30**.
