...
0. **Read first.** Study the existing surface, the relevant v2 pattern (if UI), and the matching KB entry. Write a one-paragraph brief of what you're about to do. If you can't, you're not ready to code.
    - **Write-path verification (mandatory when your code reads any RTDB or CF-response field).** For each field your code will read, grep the corresponding `set()` / `update()` / `res.json()` call in the writing CF and confirm: (a) the field is actually written, (b) the type matches your assumption (number vs string), (c) any lifecycle marker (status, completedAt, deletedAt) is where you expect, (d) any "absent field" assumption holds (e.g. "no `status` field → use `completedAt === null`"). KB docs and schema specs lie or lag — the `set()`/`update()` call is ground truth. This rule fired twice as negative confirmation on PR #72: same pattern as PR #28's `t.id`-vs-`templateId`, validated 2026-05-01 as a SELF_OPT, now codified here so the pre-flight discipline doesn't depend on remembering to read the SELF_OPT file mid-task.
...
5. **Verify.** `npm run build` must pass. For UI: manual smoke test in the browser covering golden path + at least one error path + mobile breakpoint. Do not claim "done" without running the build.
    - **Pre-push self-review (mandatory before step 7).** Before pushing the branch, re-read your diff with fresh eyes. Specifically check: (a) **shape contradictions** — does any field name/type/lifecycle in your diff contradict a recent LESSON or the actual CF write path? grep LESSONS for keywords from your code/doc assertions; (b) **mock-vs-server divergence** — every vitest mock of a server response should be copied verbatim from the server's `res.json(...)` line, with a comment citing `functions/<file>.js:<line>`; (c) **scope-discipline boundary** — when a fix-X makes a previously-dead bug-Y newly reachable, Y is in-scope (2026-05-15 inverse rule); (d) **automated-review pre-rehearsal** — what would the automated reviewer flag? Counter-fragility, doc-vs-LESSON drift, dead code blocks, info-disclosure in error strings; (e) **writer census before any RTDB `.write` tightening** (validated 3x — PR #73/#120/#125) — `rg --include=*.js --include=*.html --include=*.vue` for ALL client-SDK writers of every node whose `.write` you're tightening, and route/remove each BEFORE the rule deploys. **Inline `<script>` in HTML pages are live code** — the #125 security-review caught a missed `admin_tools/*.html` writer because the census grepped only `**.js`; an atomic multi-path `update()` carrying a now-rejected field fails the WHOLE write; (f) **review re-audit, both directions** — before merge, `gh pr view N --json reviews` and answer each finding YES/NO/PARTIAL with file:line evidence (don't trust "subsequent commits look like fixes"); after a merge that landed without you, re-read the last session's merged-PR review threads for unactioned 🔴 must-fixes, verifying each against LIVE code not the review text. The cost of this 2-minute pass is much less than a preview-fix round-trip or a reviewer must-fix. Pattern fired 3x recently (PR #67, #73, #72) where automated review or operator preview caught what a final-pass should have caught.
...
11. **Reflect.** After every PR merge (or significant code change), update the three feedback files and report findings to the user:
    ...
    - `KNOWLEDGE BASE/PROJECT_BACKLOG.md` — mark sprint tasks done, move feature to Recently Completed, log any new bugs. **Note discipline:** the header "Last updated" note is CURRENT STATE ONLY (≤10 lines; first line `Last updated: YYYY-MM-DD — <one-line note>`). Move the outgoing note paragraph to `KNOWLEDGE BASE/BACKLOG_HISTORY.md` (newest first) — never let the note grow into a changelog again (it once reached 60k tokens and made every session-start read cost most of a context window).
    - **`public/data/project-status.json` — run `npm run status:sync` in the SAME commit/PR as any `PROJECT_BACKLOG.md` body change.** The script (`scripts/generate-project-status.js`) derives `lastUpdated`/`lastUpdatedNote`/`recentlyCompleted`/`bugs` from the backlog markdown and preserves everything else; `sprint`/`phases`/`tasks` remain manual (edit the JSON directly if the sprint state changed), and `progress` is derived in `service.js` — never hand-set it. `npm run status:sync -- --check` verifies lockstep without writing (use it in the pre-push self-review). Do NOT hand-edit the derived fields — the next sync would clobber the edit.
...
| `public/data/project-status.json` | End (**same PR** as any backlog body change) | Machine-readable copy powering `/project-status.html`. **Generated — run `npm run status:sync`**, never hand-edit the derived fields (see Step 11). |
| `KNOWLEDGE BASE/BACKLOG_HISTORY.md` | End (append when the backlog note changes) | Archive of outgoing "Last updated" notes, newest first. **Not** a session-start read. |

**Session-end backlog update:** mark completed sprint tasks `[x]`, move finished features to Recently Completed, log newly discovered bugs in Bug Triage Queue, clear the In Progress row if the branch merged — then run **`npm run status:sync`** in the same commit (Step 11).

## Backlog Automation

Development runs through the backlog on three tiers:

1. **Automation queue** — `## 🤖 Next Up — Automation Queue` in `PROJECT_BACKLOG.md`: small, fully-specced task cards (scope, files, acceptance, dependencies). Only well-bounded work goes here; RTDB rules deploys, payments, prod-data sweeps, and product-lane features (W1/W2) stay in interactive sessions.
2. **Scheduled `backlog-runner`** (weekday mornings) — picks the TOP unblocked card and executes it through the Standard Task Workflow. Hard rules (also printed in the queue header): one card per run; never deploy anything; never self-merge; skip cards whose files overlap an open PR; a run is DONE only when a PR is open — work invisible to the operator doesn't exist (2026-07-05 lesson). **Dedup is merge-atomic:** the runner's PR strikes its own card row (`✅ PR #N —`), uses the branch convention `feature/queue-<card-id>`, and pre-flights merged PRs + acceptance-already-met before executing. Scheduled `backlog-groomer` (weekly) reconciles merged PRs into the backlog + Recently Completed, runs `status:sync`, prunes stale queue cards, refills the queue from the bug triage table and 3c seed list, and opens a docs PR with its changes.
3. **Operator-triggered parallel batches** — 3–4 disjoint-scope cards fanned out to worktree agents in one session, merges serialized (see Parallel Agent Work).

Throughput is bounded by operator review — automation fills the PR queue; it never merges it.
