...
Last updated: 2026-05-11 (PR #46 `feat/post-login-router-rollout` open + under review. **PR 6 folded into PR 4** — a fresh-signup bug flagged in review revealed the router's `!completed` check fired before the helloSeen check, sending post-PR42 fresh accounts to the wizard instead of the hello. Fix rewrites `resolvePostLoginDestination` with 3-state `helloSeen` semantics (false=hello, true=home/wizard-skipped, missing=legacy back-compat) — which is exactly the PR 6 work. Decision (2026-05-11): collapse PR 6 into PR 4 rather than ship a follow-up, since the router contract was incoherent without it. PR 5 (sidebar cleanup) becomes the only remaining Phase 5 PR.)
...
| Item | Branch | Notes |
|------|--------|-------|
| Phase 5 PR 4 + PR 6 (folded) — router rollout + drop wizard for new accounts | `feat/post-login-router-rollout` | **PR #46 open + under review.** Two commits: (1) wire 5 redirect sites to `routePostLogin`; (2) review-feedback fixes + fold PR 6 in (router rewrite with 3-state helloSeen semantics, hello continueHref default → `/ross.html`, dedupe RTDB reads in dashboard, AUTHENTICATION_FLOW.md sync). Build green; 21/21 router tests pass. Awaiting operator smoke-test on preview channel + merge. |
...
- [~] **PR 4 + PR 6 (folded) (PR #46)** — `feat/post-login-router-rollout`: **open + under review.** Wires the 5 remaining redirect call sites (`user-login.js` ×2, `onboarding-wizard.js` ×2, `user-dashboard.js`, `dashboard.store.js`) to the router; `user-dashboard.html` gets a sticky deprecation banner. Review surfaced a real bug — fresh signups (`completed: false && helloSeen: false`) hit the wizard because the router's `!completed` short-circuit fired before the helloSeen branch. Fix folds the PR 6 work in: `resolvePostLoginDestination` rewritten with three explicit helloSeen states — `false` → hello, `true` → home (wizard skipped, PR 6 lock), missing → legacy `!completed`-gated path. Hello component default `continueHref` updated `/onboarding-wizard.html` → `/ross.html`. AUTHENTICATION_FLOW.md gains a Post-Login Routing section. 21/21 router tests pass (was 19 — added fresh-signup + PR 6 cases). The wizard remains reachable by direct URL for legacy use; only fresh signups skip it. **This is the moment ROSS becomes home.**
- [ ] **PR 5 (≈#47)** — `feat/ross-sidebar-cleanup`: collapse sidebar to just Today + Ross's brain (3 sections). v1 module deep-links removed from ROSS sidebar; operator reaches them via `/admin-dashboard.html`. Depends on PR 4. **Becomes the only remaining Phase 5 PR after #46 merges.**
