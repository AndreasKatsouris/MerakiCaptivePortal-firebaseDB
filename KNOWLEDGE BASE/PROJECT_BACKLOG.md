...
| Item | Branch | Notes |
|------|--------|-------|
| **Run execution UI (Phase 6 PR 3)** | `feature/ross-run-execution-ui` → **PR #65** | Awaiting operator merge after preview smoke. Plan: 16 bite-sized tasks via subagent-driven-development. Implementation green across all unit tests (19/19). Five preview-smoke fix-ups applied: (1) task object→array normalization, (2) required-default predicate + yes_no boolean + Sparks header chrome, (3) `rossSubmitResponse` server contract (no `responses` map, `runCompleted` boolean, 422 body uses `flagged` not `requiredNote`), (4) `rossCreateRun` envelope unwrap. Rebased onto current master 2026-05-15. Preview: https://merakicaptiveportal-firebasedb--ross-run-execution-ui-ra5jorvd.web.app |
...
- [ ] **Operator-facing Run execution UI** (**PR #65, awaiting merge 2026-05-13**) — `?tab=run` page on the concierge shell. Polymorphic `RossRunTaskInput` covers all 10 inputTypes; auto-save on commit; client pre-flight `requiredNote` flow; inline completion banner. Spec: `docs/plans/2026-05-13-ross-run-execution-ui-design.html`. Plan: `docs/plans/2026-05-13-pr3-ross-run-execution-ui-plan.md`. Subagent-driven dev across 16 tasks; 36 unit tests across 4 new suites + updated mocks for the realistic server shapes. Five preview-smoke fix-ups (see In Progress row). Known limit: optimistic `onTime: true` on completion until first `getRun` refresh — documented in code.
