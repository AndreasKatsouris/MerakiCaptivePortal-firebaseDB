...
Last updated: 2026-05-15 (post PR #67 merge — reflect cycle. HIGH-sev `registerUser` callable bug fixed: migrated v1 `(data, context) =>` signature to v2 `(request) =>`, validated end-to-end on prod via fresh signup. Audit of the other 7 v1 callables in `functions/index.js` logged as new triage entry. Automated review caught 1 🔴 must-fix that an earlier scope-discipline over-correction had reverted out of the PR — operator's review-status probe surfaced it before merge. Phase 6 sprint: 2 items remaining.)
