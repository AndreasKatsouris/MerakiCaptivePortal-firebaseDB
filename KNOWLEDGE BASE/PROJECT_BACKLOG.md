...
Last updated: 2026-05-19 (post PR #70 merge — reflect cycle. Mid-session bug-triage pivot from Phase 6 sprint: admin dashboard's "Clear Scanning Data" button hit RTDB's WRITE_TOO_BIG (~16 MB) ceiling on a grown `/scanningData` node. Operator unblocked today via `firebase database:remove /scanningData` CLI (CLI chunks recursively, bypasses per-write limit). PR #70 shipped the durable fix: migrated `clearScanningData` from broken `onRequest` + httpsCallable-shape-mismatch to v2 `onCall` + chunked-delete (500/batch, 100K/call cap, client loops for larger nodes). Also dropped `tempClearData` (hardcoded 2024 secret, same WRITE_TOO_BIG bug, same file/concern per fold-in scope rule). Deployed + verified on prod before merge. Phase 6 sprint: 2 items remaining (active-run surfacing + stretch Compliance Sweep template).)
...
| Item | Branch | Notes |
|------|--------|-------|
| — | — | Idle. Phase 6 PR 3 merged (Run execution UI). Next sprint task: concierge home active-run surfacing — replace scripted cards on `/ross.html` with real-data cards from activated workflows. |
...
|-----|----------|------------|-----------------|
| **`/scanningData` has no retention policy** — node grew unbounded until the in-app "Clear" button hit RTDB's WRITE_TOO_BIG limit (PR #70 root cause). Meraki captive-portal scan logs accumulate continuously; only manual admin clear ever removed them. Options: (a) scheduled prune CF (`onSchedule('every 24 hours')`) using same chunked-delete pattern as `clearScanningData`, prune records older than 30/90 days; (b) TTL-on-write with `expiresAt`; (c) aggregate-then-discard to hourly/daily rollups. Needs product conversation on retention window. Also: `database.rules.json` `/scanningData` is `.write: auth != null` (any authed user can write/delete) — separate rules-audit concern. | Medium (recurrence guaranteed without retention policy) | PR #70 root-cause audit | No |
