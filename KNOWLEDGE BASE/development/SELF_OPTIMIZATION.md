...
| Inline-confirm pattern variants by destructive scope: row-inline (People delete), slide-down strip on card (Playbook workflow delete), modal (TBD HfConfirm — bulk actions only) | One pattern doesn't fit every destructive action. People's row-inline doesn't fit a workflow card with multiple meta strips; slide-down strip works there. Reserve modals for genuinely overlay-worthy cases | 2026-05-01 | 2 (PR #28, PR #30 reused for templates) |
| Read all four feedback files (PROJECT_BACKLOG + SELF_OPT + LESSONS + SCORECARD) at session start, not just PROJECT_BACKLOG | The Session Opening Protocol in CLAUDE.md only formally requires the backlog. PR #30 session: user prompted "have you reviewed your system ruleset?" and the explicit re-read of LESSONS turned the field-name-verify pattern into a hook I actually applied (verified `templateId`/`subtasks`/allowedFields against ross.js *before* writing code). Backlog alone gives the goal; the other three give the *how* | 2026-05-01 | 1 |
...
| Address minor review findings on the same branch before merge when small/in-scope; defer pre-existing or out-of-scope ones to backlog with explicit cross-reference | PR #30 review surfaced 4 findings: stable v-for keys (in-scope, fixed), empty-array validator (in-scope, fixed), CF Catalog drift (pre-existing, logged as backlog #12), rules-safe verification (manual, verified). One commit on the branch is cheaper than a follow-up PR for items that are demonstrably small; backlog items keep pre-existing drift visible without scope creep | 2026-05-01 | 1 |
