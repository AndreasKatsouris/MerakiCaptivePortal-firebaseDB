...
The project's **first AI-inference CF.** `rossChat` is the v1 reactive engine (raw Anthropic Messages API, streaming SSE): the owner asks Ross a question and Ross answers grounded in tool results, auto-executing only low-risk read/run tools (`functions/agent/`). **Four pre-flight gates** run before any LLM spend — global kill switch (`ross/config/agentKillSwitch`) → per-owner enable (`ross/agentConfig/{uid}/enabled`) → `features.rossAgent` entitlement → ledger balance; **super-admins skip the entitlement + balance gates** (still bound by the kill switch). Usage is debited once per turn via the ① ledger (`recordUsageAndDebit`, idempotent on the request id). **Confirm-flow is LIVE (slice 4, #136):** the 4 `confirm`-tier playbook-authoring tools (activate/create/edit/pause workflow) are READY — a confirm-tier call PAUSES the turn, persists a pending action, and emits a `confirm` card for the owner to approve/decline via a resume request (detailed in the `rossChat` table row below). The loop auto-runs only the READY read/run tools; confirm-tier tools always route through the pause/resume gate. **W1 capability-breadth (8 READY: 4 AUTO + 4 CONFIRM — `snoozeCard` removed 2026-07-21, snoozing is a user action):** `getFoodCostSummary` (AUTO, read-only) is the first W1 adapter — args `{ locationId }`, caller-access-checked on `userLocations/{uid}/{locationId}` before any Admin SDK read, reads `locations/{locationId}/stockUsage`, returns `{ hasData, period, dataAgeDays, foodCostPct, previousFoodCostPct, trend, salesAmount, totalCostOfUsage, itemsAnalysed, lowStockCount, lowStockItems[] }` (or `{ hasData: false }` on no-access/no-data). Read-only — does NOT run purchase-order calc (Deliverable 2). `getGuestsSummary` and `getSalesSummary` remain pending stubs. The owner-facing **Ask Ross ⌘K client (slice 5, #139)** is the live consumer. See `docs/plans/2026-05-31-askross-agent-design.md`.
...
| `rossProactiveSweep` | Scheduled (daily 07:00 SAST) | N/A (server-side) | W2 proactive nudge (deterministic — no LLM, no billing): per opted-in owner (`ross/agentConfig/{uid}/proactive.enabled === true`), reads the workflow digest and sends ONE WhatsApp `ross_daily_digest` template — worst-first findings (overdue sorted by days late, then due-today; capped at 3) with a deep link to the top finding's run page (`ross.html?tab=run&workflowId=…&locationId=…`). Gates: global kill switch → per-owner agent enable → `features.rossAgent` entitlement (RTDB `admins/{uid}/superAdmin` bypass). Idempotent per day via the `ross/proactiveLog/{uid}/{YYYY-MM-DD}` marker (`.read:false/.write:false`, server-only). Silent when nothing actionable. Channel-adapter seam (`functions/agent/sweep/channels/` — whatsapp now; telegram/sms/email = one adapter file later). Per-owner try/catch — one owner's failure never stops the sweep |
...
| `rossRecurrenceAdvance` | Scheduled (03:00 SAST daily) | n/a (scheduled) | Rolls frozen per-location `nextDueDate` forward to the current period so workflows stop being permanently covered by an old run and can reach the `overdue` bucket again. Resets task status for the new cycle and shifts task dueDates by the same delta. Honours the agent kill switch; **DRY-RUN by default** (`ROSS_RECURRENCE_ADVANCE_MODE`), write-capped, and writes `prev` audit rows to `ross/recurrenceAudit`. |
...
| `whatsappAccountStatusCheck` | Scheduled (04:00 UTC / 06:00 SAST daily) | n/a (scheduled) | Fetches the Twilio **account** resource (`client.api.v2010.accounts(sid).fetch()`) and logs ERROR when `status !== 'active'` (or the fetch itself fails) — catches the whole channel going dormant upstream, which per-message signals like `whatsappStatusCallback` can't see (a dead account never gets that far). Log-only — no RTDB write. Uses the existing `TWILIO_SID`/`TWILIO_TOKEN` env vars, no new credentials. |
