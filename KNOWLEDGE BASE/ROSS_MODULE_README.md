...
| `rossCreateTemplate` | `verifyAdmin` | Create a new template |
| `rossUpdateTemplate` | `verifyAdmin` | Update an existing template |
| `rossDeleteTemplate` | `verifyAdmin` | Delete a template |

> **Note:** Template management was previously gated to `verifySuperAdmin`. This was changed to `verifyAdmin` since the platform has a single admin role.
...
- Admin-only: all admins can create/edit/delete templates
...
| Templates gated to superAdmin | `rossCreateTemplate` / `rossUpdateTemplate` / `rossDeleteTemplate` used `verifySuperAdmin` | Changed all three to `verifyAdmin` |
