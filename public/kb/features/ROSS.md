...
**Tool catalog:** 9 **ready** (model-exposed) — 5 AUTO (auto-run, no confirm): `getWorkflowDigest`, `getStaff`, `getRunHistory`, `snoozeCard`, `getFoodCostSummary` *(reads `locations/{locationId}/stockUsage`; caller-access-checked before any read; returns food-cost %, trend, low-stock items — read-only, no purchase-order calc)*; 4 CONFIRM (playbook-authoring, pause/resume gate): `activateWorkflow`, `createWorkflow`, `editWorkflow`, `pauseWorkflow`; 11 **pending** (`status:'pending'`, defined but not exposed; `run` throws `AdapterPendingError`) — light up as their CF logic lands.
