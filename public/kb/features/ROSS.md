...
**Tool catalog:** 8 **ready** (model-exposed) — 4 AUTO (auto-run, no confirm): `getWorkflowDigest`, `getStaff`, `getRunHistory`, `getFoodCostSummary` *(reads `locations/{locationId}/stockUsage`; caller-access-checked before any read; returns food-cost %, trend, low-stock items — read-only, no purchase-order calc)*; 4 CONFIRM (playbook-authoring, pause/resume gate): `activateTemplate`, `createWorkflow`, `editWorkflow`, `pauseWorkflow`; 7 **pending** (`status:'pending'`, defined but not exposed; `run` throws `AdapterPendingError`) — light up as their CF logic lands. **`snoozeCard` was REMOVED from the agent registry 2026-07-21** — snoozing is a *user* action (UI → `rossV2Snooze` CF → `ross/v2Snoozes/{uid}`, filtered by `detectors.js`), never an agent one: it is the only action that makes information disappear, and Ross's job is surfacing what the owner would otherwise miss. It was also never grounded — it required a `cardId` no tool or context ever supplied, so the model could only call it by inventing one.
